# Hi there, I'm Raneem 👋
## Data Science & Analytics | Applied Machine Learning | Data Engineering | Business Intelligence | AI Researcher

Analytical and detail-oriented Data Science & Analytics student passionate about building
structured, explainable, and real-world AI solutions.
My interests focus on AI, healthcare, biomedical data, and data-driven decision systems.

---

### 🧠 About Me
- 🎓 Data Science & Analytics student
- 🔬 AI Researcher with interest in Explainable AI (XAI)
- 🧬 Interested in Biomedical AI & Data Visualization
- 📊 Business Intelligence & Data Storytelling
- ⚙️ Data Engineering (ETL / ELT, Data Quality)

---

### 🧩 Projects

- 🧠 **[Epileptic Seizure Prediction using EEG Signals](https://github.com/DSraneem/epileptic-seizure-prediction-eeg)**  
  Healthcare-focused AI project using EEG time-series data and machine learning.

- ⚙️ **[Weather Data ETL & ELT Pipelines](https://github.com/DSraneem/weather-etl-elt-pipeline)**  
  Data engineering pipelines integrating APIs and CSVs with validation and analytics-ready outputs.

- 📊 **[Uber Analytics Dashboard](https://github.com/DSraneem/uber-analytics-dashboard)**  
  Interactive Power BI dashboard analyzing demand, pricing, and user behavior.

- 🤖 **[Machine Learning Models Comparison](https://github.com/DSraneem/machine-learning-models)**  
  Comparison of supervised ML models with evaluation and interpretability focus.

- 📊 **[Data Mining Project](https://github.com/DSraneem/data-mining-project)**  
  Pattern discovery using clustering and classification techniques.

- 🔍 **[Information Retrieval System using TF-IDF](https://github.com/DSraneem/information-retrieval-tfidf)**  
  Text-based document ranking using NLP techniques.

- 📈 **[Big Data Analysis Project](https://github.com/DSraneem/big-data-analysis)**  
  Large-scale data processing and analytical insights.

---

### 🛠️ Skills
- **Languages:** Python, SQL
- **ML & AI:** Scikit-learn, Applied ML, XAI
- **Data Engineering:** PostgreSQL, SQLAlchemy, ETL/ELT, Great Expectations
- **BI & Visualization:** Power BI, Dashboards, KPI Storytelling

---

### 📫 Contact
- LinkedIn: https://www.linkedin.com/in/raneem-alyami-88b628238?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=ios_app
- Email:raneemalyami12@gmail.com

_"Curious by nature, research-driven by choice."_



